...
Todo lo que sigue habla de centros de distribución en general, mientras que el que se estudió y los datos que se utilizaron se describen en un capítulo propio.
...
La Tabla~\ref{tab:tipos-deposito} resume estas diferencias, que reaparecen cada vez que una decisión dependa del tipo de operación.

\begin{table}[htbp]
    \centering
    \footnotesize
    \begin{tabularx}{\textwidth}{|>{\hsize=0.95\hsize}L|>{\hsize=0.9\hsize}L|>{\hsize=1.15\hsize}L|>{\hsize=1.3\hsize}L|>{\hsize=0.55\hsize}L|>{\hsize=1.15\hsize}L|}
        \hline
        \textbf{Tipo} & \textbf{Cliente} & \textbf{Pedido típico} & \textbf{Surtido y demanda} & \textbf{Unidad que sale} & \textbf{Lo que manda} \\
        \hline
        Reposición de tiendas & tiendas de una cadena, fijas y con entregas programadas & cientos o miles de líneas, conocidas con un día o más de anticipación & cambia con el marketing, demanda agregada alta y planificable & caja o pallet & mover mucho volumen según un plan \\
        \hline
        Repuestos & concesionarios y talleres & dos corrientes, reposiciones grandes y previsibles, y urgencias de dos o tres líneas & decenas o cientos de miles de piezas, demanda individual chica e impredecible, mucho stock de seguridad & pieza & variedad, urgencia y espacio \\
        \hline
        Comercio electrónico & consumidor final & una a tres líneas, muchísimos pedidos por día, despacho inmediato & surtido amplio, demanda que se intenta moldear con precios y plazos, devoluciones & pieza & muchos pedidos chicos con plazo corto \\
        \hline
        Perecederos & comercios y consumidores & variable & permanencia de horas, salida por vencimiento, zonas de temperatura distintas & caja & frío, espacio caro y rotación forzada \\
        \hline
        Operador logístico & varios clientes en un mismo edificio & la de cada cliente & inventarios separados, estacionalidades que se complementan & mixta & escala compartida y separación de dueños \\
        \hline
    \end{tabularx}
    \caption{Tipos de centro de distribución según el cliente al que sirven, y lo que cada uno implica para el pedido, el surtido y la unidad que se despacha. Elaborado a partir de \citet[sección~1.2]{bartholdi2014warehouse} y, para las devoluciones del comercio electrónico, de \citet{boysen2019warehousing}.}
...
El argumento de Bartholdi y Hackman es que un producto controlado con precisión se mueve más rápido y con menos inventario en el sistema, una mejora que se suma a las del transporte y de la coordinación entre eslabones.

A esto se suma una consecuencia que atraviesa todo el capítulo.
...
La industria ensayó respuestas muy distintas a esa presión, desde automatizar el movimiento interno hasta repensar la organización del trabajo y del espacio, y buena parte de este capítulo está dedicada a recorrerlas.
...
Por otro lado, a la espera de que algún pedido solicite una SKU, la mercadería no consume trabajo pero sí espacio, ocupando una ubicación con dirección propia dentro del depósito.
...
Una regla atraviesa todas las etapas, y es que el producto fluya deteniéndose lo menos posible, porque cada vez que un bulto se deja en algún lugar intermedio alguien va a tener que volver a levantarlo, y esa doble manipulación, sumada sobre los cientos de miles de piezas que pasan por un depósito, se vuelve un costo considerable.
...
Formalmente es un problema de viajante de comercio, y para un depósito rectangular de un solo bloque de pasillos paralelos existe un algoritmo exacto eficiente \citep{ratliff1983order}, aunque la garantía se pierde a medida que el layout se complica con pasillos transversales.
...
La primera dice cuántas oportunidades de viaje genera esa SKU, aunque no cuántos viajes, porque cuando hay batching una lista que junta varios pedidos con la misma SKU la visita una sola vez, así que lo que cuenta como pick es la cantidad de listas de recolección que la piden, que coincide con la de pedidos solo cuando cada lista es un pedido, y cuánto se camina en cada visita depende además de dónde esté guardada, del recorrido y de qué otras SKU se buscan en la misma pasada, mientras que la segunda cuenta unidades, o su volumen y su peso, y sirve para dimensionar la manipulación, la reposición y el transporte, pero no el espacio, que depende de cuánto stock se guarda de esa SKU y no de cuánto se mueve.
...
Si se ordenan las SKU de la más pedida a la menos pedida, la curva que aparece suele ser similar a la de la Figura~\ref{fig:perfil-pareto}, unas pocas SKU concentran la mayor parte de los picks y una cola muy larga se reparte el resto, incluso en algunos casos la quinta parte de las SKU genera más de tres cuartos de los picks.
...
El propio perfil también cambia con el tiempo, casi todo producto tiene cierta estacionalidad, ya sea anual, mensual o semanal, y no siempre es la obvia, y a eso se suman las SKU nuevas, que aparecen poco pedidas solo porque llevan poco tiempo almacenadas.
...
Las dos miradas sirven para cosas distintas y se toman por separado, primero la asignación de ubicaciones a partir de lo que se sabe de cada SKU y después la afinidad a partir de lo que se sabe de sus pares.
...
De estas últimas, la asignación una por una pide almacenamiento dedicado, mientras que la de clases queda en el medio, porque reserva una zona por clase y dentro de ella deja compartir, y para elegir con cualquiera de las dos hay que empezar por medir dos cosas, qué tan lejos queda cada ubicación del punto donde empiezan y terminan los recorridos, y cuántas veces se la va a visitar, porque el costo de una asignación sale de combinar una con la otra.

Para lo primero alcanza con la distancia, si los recorridos salen del dock y vuelven a él, a cada ubicación $k$ le corresponde un costo de acceso $c_k$, la distancia de ida y vuelta, que depende solo de la geometría, y la Figura~\ref{fig:conveniencia} muestra dos ubicaciones con $c_k$ distinto en un mismo layout.
Para lo segundo sirve el perfil de actividad, porque si la SKU $i$ aparece en $f_i$ listas de recolección y está guardada en la ubicación $k(i)$, la caminata que genera es proporcional a $f_i\, c_{k(i)}$ y la de todo el depósito a
\begin{equation}
    \sum_i f_i\, c_{k(i)},
...
Con recorridos que visitan varias ubicaciones por viaje la suma deja de ser la caminata real y pasa a ser una aproximación, porque el costo de ir a una ubicación depende de cuáles otras se visitan en el mismo recorrido, y esa diferencia vuelve a aparecer en la Sección~\ref{sec:sintesis}.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[x=1cm, y=1cm]
        ...
        % un solo dock, abajo al centro
        \fill[black!70] (2.29, 0.1) circle (0.07);
        \node[font=\scriptsize] at (2.29, -0.2) {dock};
        % ubicacion A, cerca del dock
        \fill[black] (2.22, 1.0) circle (0.08);
        \node[font=\scriptsize, anchor=west] at (2.3, 1.0) {$A$};
        \draw[thick, black!75, -{Stealth}] (2.29, 0.1) -- (2.29, 1.0);
        \draw[thick, black!75, -{Stealth}] (2.36, 1.0) -- (2.36, 0.1);
        % ubicacion B, en una esquina
        \fill[black] (0.36, 2.6) circle (0.08);
        \node[font=\scriptsize, anchor=east] at (0.3, 2.6) {$B$};
        \draw[thick, black!45, dashed, -{Stealth}] (2.29, 0.1) -- (0.43, 0.3) -- (0.43, 2.6);
        \draw[thick, black!45, dashed, -{Stealth}] (0.5, 2.6) -- (0.5, 0.36) -- (2.29, 0.16);
    \end{tikzpicture}
    \caption{Dos ubicaciones y su viaje de ida y vuelta desde el dock, con $c_A < c_B$.}
    \label{fig:conveniencia}
\end{figure}

Las $f_i$ cuentan solo uno de los dos motivos por los que se visita una ubicación, por un lado la visitan los operarios que recolectan, una vez por cada lista que pide esa SKU, y por el otro la visitan los que reponen, una vez por cada vez que hay que volver a llenarla, y las dos frecuencias pueden ser muy distintas.
...
Cuando se mueven pallets enteros y cada pallet que sale se repone con otro, las tres crecen a la par, porque cada pallet que entra se guarda una vez y se saca una vez, y de ese caso viene la regla de guardar cerca lo que más rota.
...
Una medida que respeta esa diferencia es el espacio que una SKU necesita dividido por la cantidad de pedidos que la piden, conocida como \emph{cube-per-order index} o COI \citep{heskett1963cube}, que manda a las mejores ubicaciones a las SKU que dan más picks por metro ocupado.
...
Antes de medir nada conviene distinguir cuatro términos que se suelen usar de forma indistinta, pero que guardan ciertas diferencias.
\textbf{Co-ocurrencia} es un conteo, de en cuántos pedidos aparecieron dos SKU cualesquiera, luego
...
\textbf{correlación} es una medida estadística de cuánto se aleja lo que se pidió junto de lo que se esperaría si las dos demandas fueran independientes, que para dos variables de sí o no es el coeficiente $\phi$ y va de $-1$ a $1$, así como dos SKU muy pedidas pueden coincidir mucho sin estar correlacionadas, finalmente la
\textbf{afinidad} es el término que se usa en la literatura de slotting para el peso que dice cuánto conviene guardar dos SKU cerca, y no se trata de un dato sino de una decisión, porque se construye con alguna de las tres anteriores.
...
Con eso fijado, todas las medidas se arman con los mismos cuatro números, la cantidad $N$ de transacciones, sean pedidos o listas según la unidad elegida, la frecuencia $s_i$ y $s_j$ de cada SKU, es decir en cuántas de esas transacciones aparece cada una, y la co-ocurrencia $n_{ij}$, en cuántas aparecen las dos, y la Tabla~\ref{tab:medidas-afinidad} reúne algunas de las más usadas.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|c|c|L|}
        ...
        \hline
    \end{tabularx}
    \caption{Medidas de relación entre dos SKU a partir de $N$ transacciones, las frecuencias $s_i$ y $s_j$ y la co-ocurrencia $n_{ij}$.}
...
Si esos valores se usan como peso $a_{ij}$ para decidir qué acercar, con co-ocurrencia el par popular pesa 300 contra 9 y el modelo prioriza acercar a $A$ y $B$, mientras que con cualquiera de las otras medidas prioriza a $C$ y $D$, y en ninguno de los dos casos acercarlos garantiza un ahorro en cada una de esas transacciones, porque la caminata real depende de toda la lista.
...
Con cualquiera de las medidas simétricas, que son todas las de la tabla salvo la confianza, el resultado es una matriz simétrica de pesos $a_{ij}$, que en un depósito con muchas SKU y pedidos chicos suele ser además rala, porque la mayoría de los pares nunca coincide.
Las dos cosas terminan otorgando una ventaja, mientras que la simetría hace que cada par aporte un solo término al costo, con lo que alcanza con sumar sobre $i < j$, y lo ralo hace que ese costo tenga pocos términos distintos de cero, de modo que mover una SKU solo cambia los términos de sus vecinos.
Esa matriz simétrica también puede verse como grafo sin dirección, con una SKU en cada nodo y una arista pesada entre cada par que alguna vez coincidió, como el de la Figura~\ref{fig:grafo-afinidad}.
...
Las herramientas para escribir y resolver estas decisiones vienen de la optimización lineal, un programa lineal busca el mínimo de una función lineal de las variables sujeta a restricciones también lineales, y su propiedad central es que la región factible es un poliedro y el óptimo, si existe, está en alguno de sus vértices, de modo que alcanza con recorrer vértices para dar una solución.
Eso es lo que hace el método símplex, que desde que Dantzig lo propuso en 1947 resuelve en la práctica problemas con miles de variables \citep{dantzig1963linear}.
...
Como $a_{ij} = a_{ji}$ y $d_{\ell k} = d_{k \ell}$, cada par aparece dos veces en esa suma, una como $(i, j)$ y otra como $(j, i)$, lo que en un problema puramente cuadrático es solo un factor dos que no cambia el óptimo.
...
Juntar los dos términos da un problema de la forma
\begin{equation}
    \begin{aligned}
...
En el slotting los papeles son los mismos, las SKU son las plantas, las ubicaciones son los sitios, la afinidad $a_{ij}$ hace de flujo y el término lineal es el de la Ecuación~\ref{eq:asignacion-lineal}.
Escribirlo así, sin embargo, deja dos cosas abiertas.
Una es el conteo de los pares, porque aquel factor dos que no cambiaba el óptimo del término cuadrático solo, acá duplica el peso de la afinidad frente al término lineal si no se lo corrige.
La otra es la escala, porque $f_i$ cuenta picks y puede valer miles mientras que $a_{ij}$ depende de la medida elegida, por ejemplo Jaccard varía entre 0 y 1, así que sumar los dos términos tal cual deja que el más grande domine la optimización, por lo que lo habitual es llevar $f$ y $a$ a fracciones, de este modo cada suma pasa a ser una distancia promedio, y agregar un peso que diga cuánto importa una frente a la otra.
Las dos dependen de qué se elija como $a_{ij}$ y de cómo se cuenten los pares, así que no las fija la forma general sino cada formulación concreta.

El precio de esa generalidad es la dificultad, porque el problema de asignación cuadrática es NP-hard \citep{sahni1976pcomplete}, lo que quiere decir que es al menos tan difícil como cualquier problema de la clase NP, la de los problemas cuya solución se puede verificar en tiempo polinomial, así que un algoritmo que lo resolviera en tiempo polinomial daría uno para todos los problemas de esa clase, algo que se considera muy improbable, y hasta hoy no se conoce ninguno.
En la práctica los métodos exactos que existen crecen muy rápido con el tamaño de la instancia, y como a diferencia de la asignación lineal la relajación no da soluciones enteras, no hay atajo, de modo que métodos como \emph{branch and bound} \citep{land1960automatic}, que recorren el árbol de asignaciones posibles acotando el costo de cada rama y descartando las que no pueden mejorar la mejor solución conocida, solo resultan prácticos en instancias chicas, y la revisión de \citet{loiola2007qap} ubicaba ese límite en el orden de las pocas decenas de objetos.
...
Las constructivas arman la asignación de a una SKU, eligiendo en cada paso la ubicación más barata dado lo que ya se asignó, y son rápidas pero lo que entregan es solo un punto de partida, que ni siquiera tiene por qué ser un óptimo local.
Las metaheurísticas, como el recocido simulado, la búsqueda tabú y los algoritmos genéticos, agregan algún mecanismo para no quedarse en el primer óptimo local, ya sea aceptar a veces un cambio que empeora, recordar por dónde ya se pasó o trabajar con varias soluciones a la vez, y son las más usadas en el slotting con afinidad, aunque cada una trae parámetros que hay que calibrar \citep{li2015dynamic}.
...
% NOTA PARA EL CAPITULO PROPIO: la formulacion de la tesis es casi la de la Ecuacion eq:qap; alla va la instancia concreta (que es a_ij, d y c, el peso lambda, la normalizacion de f y a, las restricciones del deposito) y el punto de que la suma es un surrogate de la distancia real, a verificar por simulacion.

\section{Estado del arte}
\label{sec:estado}

Guardar juntos los productos que se piden juntos es una idea vieja, y como antecedente directo se suele citar a Frazelle y Sharp, que a fines de los ochenta propusieron la asignación correlacionada \citep{frazelle1989correlated}.
Desde entonces el problema recibió varios nombres, asignación correlacionada, slotting por afinidad o \emph{family grouping}, en un área cuya terminología las propias revisiones señalan como no unificada \citep{rojasreyes2019slap, islam2023cslap}.
La revisión de \citet{islam2023cslap} cubre sesenta trabajos entre 1985 y 2022, y además del esquema de dos fases de la Sección~\ref{sec:formulacion} marca que muy pocos validan sobre datos reales.
Lo que sigue no recorre esos trabajos uno por uno, sino que los ordena por las preguntas que cada uno tuvo que responder, y la Tabla~\ref{tab:estado-arte} los reúne.
Cuando en esta sección se dice que ninguno hace algo, se habla siempre de los trabajos incluidos en esta revisión y no de toda la literatura.

La primera pregunta es de dónde sale el peso que dice que dos SKU van juntas, y la respuesta más común es alguna de las medidas de la Sección~\ref{sec:afinidad}, sin que ninguna se haya impuesto por un argumento, ya que \citet{zhang2016correlated} recoge seis medidas distintas de correlación y \citet{li2015dynamic} combinan el lift con el conteo de soporte.
Entre los trabajos recientes aparecen el conteo de co-ocurrencia por par \citep{islam2024heuristic, debold2025storage}, las reglas de asociación junto con la clasificación ABC \citep{benaglia2024cold}, y la línea de scattered storage, que reparte copias de cada SKU por el depósito y no define un peso por par, porque la proximidad entre las SKU de una misma orden queda implícita en las órdenes con que se evalúa \citep{gamezalban2024scattered, chen2026integrated}.
Ninguno de los trabajos revisados verifica antes si la co-ocurrencia observada excede la que daría el azar dados los soportes, así que la afinidad se calcula y se usa sin preguntarse si hay algo que explotar.

La segunda pregunta es cómo se pasa de esa matriz a una asignación, y la norma es la descomposición en dos fases.
La crítica más directa viene de \citet{mirzaei2021integrated}, que en un sistema parts-to-picker alternan entre agrupar y asignar y reportan que la versión integrada supera a la secuencial en un 13\% promedio, un número que no se traslada al depósito manual pero que muestra que separar las dos decisiones tiene un costo.
Quienes atacan la formulación cuadrática completa lo hacen con heurísticas, como \citet{mantel2007order}, cuyo heurístico queda en promedio 2,14\% por encima del óptimo en 550 instancias chicas, con algoritmos genéticos \citep{li2015dynamic, guan2018genetic}, o con las variantes recientes, una que agrupa las SKU por pasillo y después ordena grupos y pasillos \citep{islam2024heuristic}, una de tres etapas con búsqueda de vecindad variable \citep{debold2025storage} y una búsqueda que decide a la vez dónde va cada SKU y cómo se recorre el depósito \citep{chen2026integrated}.
Lo que separa a unos de otros no es la familia de método sino cuánto del problema resuelven de una vez y cuánto descomponen.

La tercera pregunta es bajo qué condiciones se prueba cada método, y la primera es el tipo de sistema, porque la mayoría trabaja en picker-to-parts mientras que \citet{mirzaei2021integrated} y \citet{guan2018genetic} lo hacen en parts-to-picker, donde lo que cuesta no es caminar sino cuántos contenedores hay que traer hasta el operario.
El layout va de un solo bloque de pasillos \citep{zhang2016correlated} a varios bloques \citep{islam2024heuristic} y al zone picking secuencial \citep{debold2025storage}, la política en S domina el ruteo por su simplicidad \citep{islam2023cslap}, y solo \citet{chen2026integrated} prueban una misma asignación bajo varias políticas.
El batching casi no aparece, \citet{mantel2007order} lo nombran solo para justificar que se recolecte de a una orden por vez, con el argumento de que un batching extenso apaga el efecto de cualquier slotting, pero no lo prueban, y de los trabajos de la tabla solo \citet{benaglia2024cold} lo incluyen en su simulación.
El tamaño de orden va de una a veinte líneas, listas chicas incluso en el extremo alto.

La cuarta pregunta es contra qué se compara y con qué datos, y los baselines son casi siempre los tres de la Sección~\ref{sec:slotting}, aleatorio, rotación o COI, y clases ABC.
Los datos son casi siempre sintéticos con la correlación plantada, ya sea generando después de cada producto de una orden otro correlacionado con una fuerza que se barre entre el 2\% y el 10\% \citep{xu2020dynamic}, o manipulando la probabilidad de pedir un producto correlacionado en cinco niveles \citep{mirzaei2021integrated}, y las excepciones con datos reales son pocas, como el archivo de órdenes con el que trabaja \citet{debold2025storage}.
La escala acompaña, porque las instancias van de 12 SKU \citep{mantel2007order} a entre 300 y 500 \citep{mirzaei2021integrated} y a 1.000 \citep{zhang2016correlated}, lejos de las decenas de miles de un depósito real.

Con las condiciones a la vista los resultados se leen distinto.
La mejora de 2,08\% que \citet{zhang2016correlated} reporta sobre full-turnover viene de órdenes generadas solo por popularidad, sin correlación plantada, así que toda la co-ocurrencia que su método explota se explica por las frecuencias de cada SKU y ese 2\% es lo que se obtuvo en esas condiciones, y en el mismo trabajo las clases ABC quedan 10,28\% peor que full-turnover porque dentro de cada clase guardan al azar.
En \citet{xu2020dynamic} el ahorro cae de 10,48\% con órdenes chicas a 5,0\% con órdenes de veinte líneas, porque con listas largas hay que pasar por casi todos los pasillos y la asignación ya no reduce cuántos se atraviesan, y en \citet{mantel2007order} la ventaja de asignar con cuidado frente al aleatorio se reduce a un tercio al pasar de órdenes de dos a órdenes de diez.
\citet{mirzaei2021integrated}, en parts-to-picker, quedan 6,5\% por debajo de la clasificación ABC cuando la curva de demanda es 20/80, y sus ahorros crecen con el tamaño de orden, al revés que en \citet{xu2020dynamic}.
Eso no es una contradicción sino dos regímenes, porque en un sistema de \emph{pods} una orden más grande podría dar más oportunidades de que las SKU pedidas compartan contenedor, mientras que en picker-to-parts obliga a pasar por más pasillos.
Por eso la mejora porcentual por sí sola no compara trabajos si cambian el sistema, el ruteo, el tamaño de orden, los datos o el baseline, que es lo que la Tabla~\ref{tab:estado-arte} deja ver, y la Sección~\ref{sec:sintesis} recoge de todo esto cuándo la afinidad rinde y qué queda sin resolver.

% Filas de Islam 2024 y Gamez Alban 2024 confirmadas contra el texto completo; Debold 2025, Benaglia 2024 y Chen 2026 solo contra el resumen (pendiente leerlos).
\begin{table}[tp]
    \centering
    \scriptsize
    ...
    \begin{tabularx}{\textwidth}{|L|L|L|L|L|L|}
        \hline
        \textbf{Trabajo} & \textbf{Sistema} & \textbf{Señal} & \textbf{Método} & \textbf{Datos y baseline} & \textbf{Mejora reportada} \\
        \hline
        \citet{mantel2007order} & n/d, módulo vertical de bandejas, 12 SKU & n/d & heurístico QAP, una orden por vez & sintéticos, 550 instancias; óptimo y aleatorio & gap al óptimo 2,14\%; la ventaja sobre el aleatorio cae a un tercio con órdenes de 10 \\
        \hline
        \citet{li2015dynamic} & n/d & lift y soporte & QAP con afinidad y ABC, genético & caso de estudio & 7,14\% a 104,48\% en tiempo de picking \\
        \hline
        \citet{zhang2016correlated} & picker-to-parts, un bloque, 1.000 SKU & correlación por pares, medida n/d & estrategia correlacionada & sintéticos sin correlación plantada; full-turnover y ABC & 2,08\% menos distancia; ABC 10,28\% peor que full-turnover \\
        \hline
        \citet{xu2020dynamic} & picker-to-parts, ruteo en S, órdenes de hasta 20 & n/d & asignación dinámica entre \emph{waves} & sintéticos, correlación plantada de 2\% a 10\% & 10,48\% con órdenes chicas, 5,0\% con órdenes de 20 \\
        \hline
        \citet{mirzaei2021integrated} & parts-to-picker, 300 a 500 SKU & score de afinidad & agrupar y asignar integrado & sintéticos, cinco niveles; dos fases y ABC & 13\% sobre dos fases; 6,5\% peor que ABC con curva 20/80 \\
        \hline
        \citet{islam2024heuristic} & picker-to-parts, varios bloques & conteo de co-ocurrencia por par & agrupa por pasillo y ordena grupos y pasillos & sintéticos, 100.000 órdenes; full-turnover, ABC y la estrategia de Zhang & 22,55\% a 39,67\% menos distancia que full-turnover \\
        \hline
        \citet{debold2025storage} & picker-to-parts, zone picking & co-ocurrencia & tres etapas con VNS & reales; benchmarks & hasta 19,5\% \\
        \hline
        \citet{benaglia2024cold} & depósito frío, con batching y ruteo & reglas de asociación y ABC & simulación de 8 estrategias & simulados de órdenes reales; aleatorio & hasta 8\% menos tiempo de picking \\
        \hline
        \citet{gamezalban2024scattered} & picker-to-parts, scattered storage & implícita en las órdenes & VNS & sintéticos, 68 instancias con clases ABC; scatter aleatorio y por volumen & hasta 36\% y 56\% menos distancia \\
        \hline
        \citet{chen2026integrated} & picker-to-parts, scattered, tres ruteos & implícita en las órdenes & MIP y búsqueda adaptativa con ruteo & n/d; práctica estándar & hasta 14\% menos en el objetivo, distancia ponderada de guardado y picking \\
        \hline
    \end{tabularx}
    \caption{Condiciones y resultados de los trabajos revisados. Las celdas n/d son las que la fuente no informa o no pudieron verificarse, y la última columna solo se lee junto con las anteriores.}
...
Lo que la Sección~\ref{sec:estado} deja, leída en conjunto, es que el aporte de la afinidad depende fuertemente de las condiciones en que se la usa, y la evidencia permite señalar cuatro, cada una con su propio mecanismo.
Rinde cuando los pedidos son chicos y se recolectan de a uno o en lotes chicos, porque una lista larga pasa por casi todos los pasillos de todos modos y acercar dos SKU deja de ahorrar pasillos, que es lo que \citet{xu2020dynamic} miden al agrandar la orden en picker-to-parts con ruteo en S, y lo que \citet{mantel2007order} asumen, sin probarlo, cuando dicen que un batching extenso apaga cualquier slotting.
Hace falta además que la política de ruteo deje lugar al ahorro, porque con una política en S estricta lo que cuenta es en cuántos pasillos se entra y no dónde está cada SKU dentro del pasillo, y cabe esperar, aunque ningún trabajo revisado lo mida, que con retorno o con recorridos que se adaptan a la lista la posición fina vuelva a importar.
Tampoco alcanza si la curva de demanda está tan sesgada que ordenar por picks ya resuelve casi todo, porque si unas pocas SKU concentran la actividad, guardarlas cerca del dock captura la mayor parte del ahorro posible y la afinidad solo puede repartir lo que queda.
Eso es lo que \citet{mirzaei2021integrated} encuentran con una curva 20/80, aunque en un sistema parts-to-picker.
Y rinde poco si la co-ocurrencia observada no es más que un reflejo de la popularidad, porque dos SKU muy pedidas coinciden mucho por azar, y una afinidad construida sobre eso apenas reordena lo que la rotación ya ordenó, que es el 2\% que \citet{zhang2016correlated} obtiene con órdenes generadas sin correlación.
Ninguna de esas cuatro condiciones la documentó un solo trabajo, cada una aparece en uno distinto, en sistemas y con ruteos que no siempre coinciden y no siempre se informan, y ninguno de los trabajos revisados las barrió juntas, así que el umbral a partir del cual conviene pasar de la rotación a la afinidad sigue sin estar medido.

A esas cuatro se les suman otras variables que pesan tanto o más sobre lo que la afinidad puede dar, y que los trabajos de la Tabla~\ref{tab:estado-arte} casi no miran.
Una es la congestión, que la Sección~\ref{sec:slotting} ya presentó como costo escondido de la rotación y que con la afinidad podría agravarse, aunque esto es un razonamiento y no un resultado, porque juntar las SKU que se piden juntas también juntaría a los operarios que las buscan.
Por eso una asignación con menos distancia teórica podría resultar peor en la práctica si concentra el tráfico en unos pocos pasillos, y ninguno de los trabajos revisados mide esperas.
Otra es el costo de mover la mercadería hasta la asignación nueva, el mismo que la Sección~\ref{sec:slotting} describió al hablar de mantener una asignación, y que de los trabajos revisados solo \citet{xu2020dynamic} incluyen en la cuenta, al reubicar entre \emph{waves} y cargar al tiempo del operario el sacar y volver a colocar cada producto.
A eso se agrega la reposición, que sigue ocurriendo después de asignar y que con SKU agrupadas por afinidad habría que adaptar, algo que \citet{mirzaei2021integrated} señalan para su sistema parts-to-picker sin cuantificarlo y dejan como trabajo futuro junto con la asignación dinámica.
Y queda la estabilidad en el tiempo, porque la asignación se calcula con un historial y se usa hacia adelante, lo que supone que los pares que hoy van juntos van a seguir yendo juntos meses después, un supuesto que ningún trabajo revisado mide, aunque la Sección~\ref{sec:perfil} ya mostró que la demanda de cada SKU por sí sola cambia con la estacionalidad y con las altas.
A todo esto hay que agregarle que el efecto del tamaño de orden cambia de signo entre los dos trabajos que lo miden, uno en picker-to-parts y otro en parts-to-picker, como la Sección~\ref{sec:estado} ya mostró, así que cualquiera de estas condiciones se lee dentro de un régimen y no se traslada al otro.

Que esas variables falten no es casualidad de cada trabajo sino un patrón de cómo se estudia el problema.
Casi todo se mira de a dos SKU, la simplificación que la Sección~\ref{sec:afinidad} ya señaló, y aunque \citet{islam2023cslap} lo marcan como hueco abierto, casi ninguno de los trabajos revisados usa conjuntos de tres o más productos para asignar.
Los datos son casi siempre sintéticos con la correlación plantada, como mostró la Sección~\ref{sec:estado}, de modo que la pregunta de si hay señal que explotar viene respondida por construcción, y la evaluación se hace casi siempre sobre un slotting estático, muchas veces en un solo bloque de pasillos, sin esperas, sin reposición y con una afinidad que no cambia en el tiempo.
Cada una de esas decisiones es razonable por separado, porque aísla el efecto que se quiere medir, pero todas parecen ir en la misma dirección, hacia una señal más nítida y un depósito más quieto que uno en operación, y por eso las mejoras reportadas se leen mejor como lo que la afinidad podría dar bajo esas condiciones que como lo que da en operación.
Y, como la Sección~\ref{sec:estado} ya marcó, ninguno de los trabajos revisados contrasta la co-ocurrencia observada contra la que darían SKU pedidas de forma independiente con los soportes observados, que es lo que separaría una relación real de una coincidencia explicada por las frecuencias, como la de \citet{zhang2016correlated}.

De ahí salen los problemas que siguen abiertos, y el primero es la escala, porque las instancias de la Tabla~\ref{tab:estado-arte} van de la docena al millar de SKU, con órdenes de hasta veinte líneas, mientras que un depósito de comercio electrónico puede tener decenas de miles de SKU y, cuando hay batching, listas de recolección que juntan varios pedidos, y no está claro que los métodos ni las conclusiones sobrevivan al salto.
También falta validar la señal antes de usarla, comparando la co-ocurrencia observada con la que se obtendría si cada lista se armara al azar conservando la frecuencia de cada SKU y el tamaño de cada lista, y rompiendo solo la relación entre SKU, una comparación que ninguno de los trabajos revisados hace y que decide si hay algo que optimizar.
Tampoco está resuelta la afinidad dinámica, tanto en el sentido de cuánto dura una afinidad medida como en el de cómo reasignar cuando cambia, porque entre los trabajos revisados casi no aparece, ya que \citet{xu2020dynamic} reasignan entre \emph{waves} con datos sintéticos y \citet{mirzaei2021integrated} lo dejan como trabajo futuro.
Otro costo sin medir es el de descomponer en dos fases, que \citet{mirzaei2021integrated} cuantificaron en parts-to-picker pero que en el depósito manual, donde la descomposición es la norma, ninguno de los trabajos revisados mide.
Y queda la elección del peso $a_{ij}$, que la Sección~\ref{sec:afinidad} mostró que no es una cuestión de escala sino de qué objetivo se minimiza, y que en los trabajos revisados casi siempre se toma por convención y rara vez se discute qué pares termina acercando.

Esta tesis se ubica en esos huecos, ya que trabaja sobre un depósito manual picker-to-parts de comercio electrónico real, con decenas de miles de SKU y listas de recolección que juntan varios pedidos, es decir en la escala y en el régimen que los trabajos de la Tabla~\ref{tab:estado-arte} no cubren.
Mide la señal antes de usarla, para saber si la co-ocurrencia de sus listas excede la de ese modelo nulo, y compara la afinidad contra la rotación bajo las condiciones del propio depósito, con su ruteo, su batching y su curva de demanda, y no bajo condiciones armadas para aislar el efecto.
Se pregunta, por último, si una mejora en la suma de la Ecuación~\ref{eq:qap} se traduce en distancia recorrida, que es la diferencia entre el objetivo y lo que se quiere bajar que la Sección~\ref{sec:slotting} dejó planteada.
Para eso hace falta primero conocer el depósito y los datos con que se hizo todo lo anterior, que es lo que sigue.
