...
El comercio electrónico creció de forma sostenida durante la última década, y con él la exigencia sobre la infraestructura que lo sostiene \citeme{crecimiento del e-commerce}.
...
En la mayoría de los depósitos el \emph{picking} sigue siendo una actividad manual, bajo un esquema conocido como \emph{picker-to-parts}, es el operario quien se desplaza hasta la ubicación del producto, y no el producto el que llega hasta él \citeme{predominancia de sistemas picker-to-parts}.
Bajo ese esquema el \emph{picking} concentra la mayor parte del costo operativo del depósito, y dentro del \emph{picking} la mayor parte del tiempo no se va en tomar los artículos ni en empaquetarlos, sino en caminar entre ubicaciones \citeme{participacion del picking en el costo operativo, y del desplazamiento dentro del picking}.
Esa caminata depende directamente de cómo estén distribuidos los productos en el inventario.

\section{El problema}

La distancia que recorre un operario no depende de una sola decisión, sino de cuatro que se toman en momentos y niveles distintos \citeme{taxonomia de decisiones de diseno y operacion de depositos}.
...
En la literatura el problema se conoce como \emph{storage location assignment problem} (SLAP), y ponderar ambas señales en una única función de costo lo ubica dentro de la familia del \emph{quadratic assignment problem} (QAP) \citeme{revision de literatura del SLAP, y la formulacion de Koopmans y Beckmann}.
Esa familia es NP-hard\completar{aca creo que iria una cita, pero seguro hablo de esto despues}, de modo que a la escala de un depósito real, con decenas de miles de productos, resolverla de forma exacta queda fuera de alcance.

El trabajo no busca entonces la solución óptima, sino optimizar un problema similar y tratable que la aproxime.
Lo que se mide después es la distancia efectivamente recorrida sobre un período posterior al utilizado para estimar las señales, comparada con la que produce la asignación vigente del depósito.\completar{esto no me gusta, poner algo como que generalice o demas, pero son las 3am}
